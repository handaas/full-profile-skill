...
The full-profile scenario needs cross-domain data from 8 MCP servers:
  enterprise / enterprise-risk / enterprise-operation / patent / trademark /
  bidding / news / recruitment.
...
SERVER_REGISTRY: Dict[str, str] = {
    ...
    "patent": "patent-mcp-server",
    "trademark": "trademark-mcp-server",
    "bidding": "bidding-mcp-server",
...
REMOTE_URL_ENV: Dict[str, str] = {
    ...
    "patent": "PATENT_MCP_URL",
    "trademark": "TRADEMARK_MCP_URL",
    "bidding": "BIDDING_MCP_URL",
    "news": "NEWS_MCP_URL",
...
SKILL_MCP_TOKEN = "FULL_PROFILE_MCP_TOKEN"
...
class MultiMcpClient:
    ...
    async def ensure_session(self, domain: str) -> Any:
        ...
        if not conn:
            raise MultiMcpError(
                f"未配置 MCP 连接 [{domain}]。请设置 FULL_PROFILE_MCP_TOKEN 或 {REMOTE_URL_ENV.get(domain, 'HANDAAS_MCP_URL')}（Remote），"
    ...
    async def warmup(self, domains: list[str]) -> None:
        ...
        Spawning 8 stdio MCP servers at once can exhaust resources or trip the
